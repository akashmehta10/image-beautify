...
public class ImgBeautify {
    ...
    public static void generateImageGradient(String fileLocation, Patterns pattern) {
        int width = 1024;
        int height = 768;
        ...
        if(pattern.equals(Patterns.GRADIENTSHAPES)) {
            System.out.println(pattern.toString());
            int randomShapesCount = 0;
            while(randomShapesCount < 500) {
    ...
    public static void insertRandomCircle(BufferedImage img) {
        ...
        public static void generateRusticImage(String fileLocation) {
            int width = 1024;
            int height = 768;
            //create buffered image object img
            BufferedImage img = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
            ...
            r1 = (int) (Math.random() * 256); //red
            g1 = (int) (Math.random() * 256); //green
            b1 = (int) (Math.random() * 256); //blue

            for (int x = 0; x < width; x+=(int) (Math.random() * 5)) {
                for(int y = height - 1; y>=0; y-=(int) (Math.random() * 5)) {
                    img.setRGB(x, y, new Color(r1, g1, b1, a1).getRGB());
                }
        ...
        if(pattern.equals(Patterns.GRADIENTPLAIN) || pattern.equals(Patterns.GRADIENTSHAPES)) {
            generateImageGradient(fileLocation, pattern);
        }
        else if(pattern.equals(Patterns.RUSTIC)) {
            generateRusticImage(fileLocation);
        }
        else{
